...
(function () {
  ...
  function processSearchResults() {
    ...
    articles.forEach((article) => {
      ...
      if (journalNameElement) {
        let journalName = journalNameElement.textContent.trim();
        const dotIndex = journalName.indexOf('.');
        if (dotIndex > 0) {
            journalName = journalName.substring(0, dotIndex).trim();
        }
  ...
  async function processSimilarArticles() {
      ...
      for (const article of similarArticles) {
          ...
          if (citationEl) {
               let text = citationEl.textContent.trim();
               const dotIndex = text.indexOf('.');
               if (dotIndex > 0) journalName = text.substring(0, dotIndex).trim();
          }
